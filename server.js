...
async function getFflogsEventsGql(report, fight, source, start, end) {
	...
	const filterExpression = String.raw`type=\"targetabilityupdate\" or ability.name in (\"Chain Stratagem\",\"Vulnerability Up\",\"Trick Attack\")`;
	const body2 = `
	query q {
		reportData {
			report(code: "${report}") {
	...
	// Safely access the data arrays, providing empty arrays as fallbacks
	const events1 = data?.data?.reportData?.report?.events?.data || [];
	const events2 = data2?.data?.reportData?.report?.events?.data || [];
